...
- [x] **/settings - theme picker** - Client-side theme system. Base.html bootstraps CSS vars from localStorage before paint (no flash). 6 presets (Newsprint Classic, FT Salmon, NYT Modern, WSJ Hunter, Le Monde, Editorial Noir), 6 color pickers, 3 font selects, 4 newspaper-flair toggles (masthead-style header, small-caps nav, accent rule under H1, rule weight slider), live preview. Per-browser storage. Email bulletins keep hardcoded styles for client compat - 2026-05-02

### v0.2 - Bug fixes (2026-05-04)

- [x] **Fix finalize.py SyntaxError + r[composed] NameError** - Two bugs from 2026-05-01 build sessions heredoc-strftime corruption (T17 had flagged it but fix did not fully land). L46 strftime(%B %-d, %Y) raised SyntaxError at module import; L56 r[composed] would have raised NameError after import fix. Sun 9pm finalize cron crashed every weekend since first scheduled run; first weekend bulletin (id=1, week 2026-05-03, 39KB / 44 stories from 2026-05-02 PM validation run) sat composed but was never emailed. Patched both lines on SV-04 + rebuilt + manually sent existing bulletin via direct python.send_bulletin call - Gmail msg id 19df2f6cb6e90590. whipple/ audited for similar unquoted-strftime / bare-name f-string bugs - clean (only finalize.py was hit by the heredoc corruption). Commit cb7e255 - 2026-05-04
